...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'LVT_CONTENT_READY') {
    ...
    console.log(`[BG:R15] hello ${parts.join(' ')}`);

    if (stage === 'viewer_node_detected') {
      lastViewerUpdateAt = Date.now();
    }

    sendResponse?.({ ok: true, stage, tabId });
    return;
  }
